#!/usr/bin/env python3
"""
FastAPI backend for the PDF chatbot UI, fully local with Gemma/Ollama.
...
and Gemma calls still live where they already did.
...
No external API keys or cloud services required -- everything runs locally.
"""
import logging
import os
import tempfile
...
def startup() -> None:
    chat_memory.init_db()
    logging.info("PDF chatbot backend started (fully local, Gemma-powered)")
...
async def upload_pdf(file: UploadFile = File(...)):
    ...
    try:
        rag = RAGWithGuardrails()
        rag.load_document(tmp_path)  # extracts text + generates summary via local Gemma
